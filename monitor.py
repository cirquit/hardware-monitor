...
class Monitor:
    def __init__(self, cuda_enabled=False):
        """
        A hardware monitor with CUDA logging support over pynmvl/torch and psutil.
        Tracks bandwidth between `get_sys_info()` calls. 
        Example:
        ...
        # static info (call once, does not change)
        static_info = mon.get_static_info()
        
        # changing information, e.g., every DL inference/training step
        for _ in range(1,100):

            system_info = mon.get_sys_info()
            time.sleep(1)
        ```

        :param cuda_enabled: (default: False) if enabled, logs various memory metrics and most of whatever nvidia-smi logs
        """
    ...
    def get_nvidia_smi_info(self):
        ...
        for ix, handle in enumerate(self.gpu_handles):
            ...
            return_dict[group_key + "temp_in_C"] = temp_in_c
             ...
             # power
            return_dict[group_key + "power_in_W"] = nvmlDeviceGetPowerUsage(handle) / 1000 # in mW originally
